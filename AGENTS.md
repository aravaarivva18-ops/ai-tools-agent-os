...
- **Неинтерактивный agy**: При фоновом неинтерактивном вызове `agy` необходимо передавать промпт строкой в флаг `--print` и принудительно перенаправлять ввод: `< /dev/null 2>&1`. В противном случае `agy` видит доступный TTY терминала и зависает на попытке инициализировать TUI или ожидать ввода.
- **Локации документации**: Хранение документации в Obsidian: `vault/` (только PRD, задачи, КП, лиды, хэндоффы), `wiki/` (глобальный RAG для VK/Bitrix API), `tools/wiki/` (инженерия, ComfyUI).
