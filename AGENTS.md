...
- **Валидация регламентов (Check Rules)**: При изменении любых регламентов или файлов навыков агент обязан запускать `make check-rules` для проверки целостности ссылок и синхронизации JIT-навыков. Неиспользуемые правила и навыки должны своевременно удаляться (YAGNI).
