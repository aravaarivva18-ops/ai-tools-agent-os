...
```bash
cd /Users/rus/ai-tools/dashboard-hand-on-pulse
...
### 2. Конфигурация (.env)
Создайте файл `.env` в корне папки `dashboard-hand-on-pulse/` с настройками:
```env
...
```bash
uv run pytest dashboard-hand-on-pulse/
```
