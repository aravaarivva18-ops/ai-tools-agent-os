...
class Project(Base):
    ...
    source_mapping = relationship(
        "SourceMapping",
        uselist=False,
        back_populates="project",
        cascade="all, delete-orphan",
    )
    marketing_facts = relationship(
        "MarketingFact", back_populates="project", cascade="all, delete-orphan"
    )
    marketing_plans = relationship(
        "MarketingPlan", back_populates="project", cascade="all, delete-orphan"
    )
    ...
    # Настройки НДС
    vat_type = Column(
        String, nullable=False, default="with_vat"
...
class MarketingFact(Base):
    ...
    leads_primary = Column(
        Integer, default=0
    )  # Количество лидов (конверсий) (вместо leads)
    leads_qualified = Column(
        Integer, default=0
    )  # Количество квалифицированных лидов (вместо qualified_leads)
