...
def extract_component_frames(
    strip: Image.Image, frame_count: int
    ...
    seeds = [
        component for component in components if component["area"] >= seed_threshold
    ]
    if len(seeds) < frame_count:
        seeds = sorted(
            components, key=lambda component: component["area"], reverse=True
        )[:frame_count]
    if len(seeds) < frame_count:
        return None

    seeds = sorted(
        sorted(seeds, key=lambda component: component["area"], reverse=True)[
            :frame_count
        ],
...
def extract_state(
    ...
    if method in {"auto", "components"}:
        ...
        if frames is None and method == "components":
            raise SystemExit(
                f"could not find {frame_count} sprite components in {strip_path}"
            )
...
def main() -> None:
    ...
    print(
        json.dumps(
            {"ok": True, "frames_root": str(output_dir), "states": states}, indent=2
        )
    )
