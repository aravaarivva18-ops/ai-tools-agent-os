...
def validate_hash(
    job: dict[str, object], *, source: Path, output: Path, job_id: str
) -> None:
    ...
    if source_hash != expected_hash:
        raise SystemExit(
            f"job {job_id} source image hash does not match imagegen-jobs.json"
        )
...
def validate_completed_job_source(
    ...
    *,
    ...
    source = manifest_path(
        job.get("source_path"), run_dir=run_dir, field="source_path", job_id=job_id
    )
    output = manifest_path(
        job.get("output_path"), run_dir=run_dir, field="output_path", job_id=job_id
    )
...
def review_failures(review: dict[str, object]) -> list[str]:
    ...
    for row in rows:
        ...
        if isinstance(errors, list) and errors:
            failures.append(
                f"{row.get('state')}: {'; '.join(str(error) for error in errors)}"
            )
    return failures
...
def main() -> None:
    ...
    parser.add_argument(
        "--allow-synthetic-test-sources", action="store_true", help=argparse.SUPPRESS
    )
    ...
    if not pet_id or not display_name or not description:
        raise SystemExit(
            "pet_request.json is missing pet_id, display_name, or description"
        )
    ...
    if not args.skip_package:
        ...
        if args.package_dir:
            package_command.extend(
                ["--output-dir", str(Path(args.package_dir).expanduser().resolve())]
            )
        run(package_command)
