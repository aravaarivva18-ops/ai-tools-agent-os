...
def next_archive_path(archive_dir: Path, state: str, attempt: int, suffix: str) -> Path:
    ...
    while True:
        candidate = (
            archive_dir / f"{state}-attempt-{attempt}-previous-{counter}{suffix}"
        )
        if not candidate.exists():
            return candidate
        counter += 1
...
def archive_decoded_output(
    run_dir: Path, job: dict[str, object], state: str, attempt: int
) -> str | None:
...
def queue_repair(
    manifest: dict[str, object], run_dir: Path, state: str, reason: str
) -> dict[str, object]:
