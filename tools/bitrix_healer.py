...
def heal_file(file_path: str) -> bool:
    ...
    for idx, line in enumerate(lines):
        ...
        new_line, count = re.subn(
            r"(?<![a-zA-Z0-9_\\])EventManager::",
            r"\\Bitrix\\Main\\EventManager::",
            line,
        )
        if count > 0:
            lines[idx] = new_line
...
def main():
    parser = argparse.ArgumentParser(
        description="Bitrix EventManager conflict healer tool."
    )
    parser.add_argument(
        "file_path", help="Path to the problematic PHP file (e.g., init.php)"
    )
    ...
    try:
        ...
        if healed:
            print(
                f"SUCCESS: File {args.file_path} healed. Backup saved at {args.file_path}.bak"
            )
        else:
            print(
                f"INFO: No conflicts or duplicate EventManager imports/usages found in {args.file_path}."
            )
