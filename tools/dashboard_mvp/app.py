...
def render_selector_panel():
    st.markdown("<div class='filter-panel'><div class='filter-title'>Панель селекторов (Фильтры)</div>", unsafe_allow_html=True)
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        client = st.selectbox("Земля (Клиент)", ["ООО Парковка-Уфа"], label_visibility="visible")
    with col2:
        project = st.selectbox("чёрный (Проект)", ["Парковка Уфа (PR-001)"], label_visibility="visible")
    with col3:
