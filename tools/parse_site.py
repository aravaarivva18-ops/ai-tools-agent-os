...
def main():
    ...
    text = parser.css_first("body").text(separator=" ") if parser.css_first("body") else html

    phones = set(re.findall(r'\+?[78]\s?\(?\d{3}\)?\s?\d{3}[-\s]?\d{2}[-\s]?\d{2}', text))
    ...
    for a in parser.css("a[href]"):
        ...
        if "wa.me" in href or "whatsapp" in href or "t.me" in href or "telegram" in href or "vk.com" in href:
