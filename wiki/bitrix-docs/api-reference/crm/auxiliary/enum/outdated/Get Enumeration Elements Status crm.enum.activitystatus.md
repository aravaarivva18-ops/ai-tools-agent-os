...
tags:
  ...
  - api
  - docs
title: "Get Enumeration Elements "Status" crm.enum.activitystatus"
original_path: "api-reference/crm/auxiliary/enum/outdated/crm-enum-activity-status.md"
---
...
- JS (TS)
    ...
    // Shape of each ActivityStatusItem returned in result[]
    type ActivityStatusItem = {
      ID: number
      NAME: string
    ...
    try {
      const response = await $b24.actions.v2.call.make<ActivityStatusItem[]>({
      ...
      if (!response.isSuccess) {
        ...
        console.info('Activity statuses:', result.length, result)
      }
    } catch (error) {
...
- JS (UMD)
    ...
    <script>
      ...
      document.addEventListener('DOMContentLoaded', getActivityStatuses)
    </script>
    ```
