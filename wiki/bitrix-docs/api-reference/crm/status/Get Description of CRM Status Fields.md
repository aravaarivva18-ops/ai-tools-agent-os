...
tags:
  ...
  - api
  - docs
title: "Get Description of CRM Status Fields"
original_path: "api-reference/crm/status/crm-status-fields.md"
---
...
- JS (TS)
    ...
    type CrmStatusFields = Record<string, CrmStatusFieldDescription>

    type CrmStatusFieldDescription = {
      type: string
      isRequired: boolean
      isReadOnly: boolean
      isImmutable: boolean
    ...
    try {
      const response = await $b24.actions.v2.call.make<CrmStatusFields>({
      ...
      if (!response.isSuccess) {
        ...
        console.info('Status fields:', Object.keys(result))
      }
    } catch (error) {
