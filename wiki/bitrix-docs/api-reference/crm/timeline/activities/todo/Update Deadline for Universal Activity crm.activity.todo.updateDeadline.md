...
tags:
  ...
  - api
  - docs
title: "Update Deadline for Universal Activity crm.activity.todo.updateDeadline"
original_path: "api-reference/crm/timeline/activities/todo/crm-activity-todo-update-deadline.md"
---
...
- JS (TS)
    ...
    type UpdateDeadlineResult = {
      id: number
    }

    try {
      const response = await $b24.actions.v2.call.make<UpdateDeadlineResult>({
...
- JS (UMD)
    ...
    <script>
      async function updateTodoDeadline() {
        try {
      ...
      document.addEventListener('DOMContentLoaded', updateTodoDeadline)
    </script>
    ```
