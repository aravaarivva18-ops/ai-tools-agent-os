...
tags:
  ...
  - api
  - docs
title: "Get a list of custom activity types crm.activity.type.list"
original_path: "api-reference/crm/timeline/activities/types/crm-activity-type-list.md"
---
...
- JS (TS)
    ...
    type ActivityTypeItem = {
      TYPE_ID: string,
      NAME: string,
      IS_CONFIGURABLE_TYPE: string,
      ICON_ID: string,
    }

    try {
      // crm.activity.type.list returns a single page (max 50 records). For the whole result set
      ...
      if (!response.isSuccess) {
        ...
        console.info('Activity types:', result.length, result)
      }
    } catch (error) {
...
- JS (UMD)
    ...
    <script>
      ...
      document.addEventListener('DOMContentLoaded', fetchActivityTypeList)
    </script>
    ```
...
- Python
    ...
    try:
        bitrix_response = client.crm.activity.type.list().response
