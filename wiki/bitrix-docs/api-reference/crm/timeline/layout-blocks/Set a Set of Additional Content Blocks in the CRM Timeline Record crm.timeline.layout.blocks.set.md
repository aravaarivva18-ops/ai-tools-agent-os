...
tags:
  ...
  - api
  - docs
title: "Set a Set of Additional Content Blocks in the CRM Timeline Record: crm.timeline.layout.blocks.set"
original_path: "api-reference/crm/timeline/layout-blocks/crm-timeline-layout-blocks-set.md"
---
...
- JS (TS)
    ...
    type TimelineLayoutBlocksSetResult = {
      success: boolean
    }

    try {
      const response = await $b24.actions.v2.call.make<TimelineLayoutBlocksSetResult>({
...
- JS (UMD)
    ...
    <script>
      async function setTimelineLayoutBlocks() {
        try {
      ...
      document.addEventListener('DOMContentLoaded', setTimelineLayoutBlocks)
    </script>
    ```
