...
tags:
  ...
  - api
  - docs
title: "Get Information About the Logo crm.timeline.logo.get"
original_path: "api-reference/crm/timeline/logmessage/logo/crm-timeline-logo-get.md"
---
...
- JS (TS)
    ...
    try {
      const response = await $b24.actions.v2.call.make<LogoGetResult>({
      ...
      if (!response.isSuccess) {
        ...
        console.info('Logo:', result.logo.code, result.logo.fileUri)
      }
    } catch (error) {
...
- JS (UMD)
    ...
    <script>
      async function getTimelineLogo() {
        try {
      ...
      document.addEventListener('DOMContentLoaded', getTimelineLogo)
    </script>
    ```
