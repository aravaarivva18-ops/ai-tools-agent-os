...
tags:
  ...
  - api
  - docs
title: "Widget Integration Mechanism"
original_path: "api-reference/widgets/index.md"
---
...
For [mass-market solutions](../../market/preparing-to-publish/how-to-add-app.md) hosted in the Developer's area, you need to do two things:
...
Unlike the integration of the slider with the main URL of the application in the left menu, [all other widgets](./placements.md) are integrated differently—using the [`placement.bind`](./placement-bind.md) method.
...
To use this scenario, you need to register [custom field types](../crm/universal/user-defined-fields/userfield-type.md). These are not the fields themselves, but rather the types. Later, either the user through standard CRM interfaces or your application using REST methods can add fields of these types and use your editing interface for such fields.

## Opening Standard and Custom Sliders from Widget Interfaces

Often, it is necessary to [open standard pages](./bx24-widget-methods.md) of Bitrix24 from the widget interface. This could be, for example, a slider with a contact card or a slider with the required task.

In addition to standard sliders, solution developers have the option to display [their custom interfaces in a slider](./bx24-widget-methods.md). For instance, this could be a custom document viewing card, a slider with application settings, a slider with a report, etc. You will be able to customize the appearance of such a slider.
...
- in the [call card](./ui-interaction/index.md)
- in the [WebRTC scenario](./ui-interaction/page-background-worker/webrtc-scenario.md)
- in the [CRM card](./ui-interaction/index.md)
...
It is also recommended to use the [openPath](./bx24-widget-methods.md) method to show users standard Bitrix24 objects, and the [openApplication](./bx24-widget-methods.md) method to simplify user scenarios within your application. Users are accustomed to the pattern where additional information or details about something are displayed in a slider over the previous interface.
